...
main_application.geometry('1200x650')
...
# ======== color theme
light_defaule_icon = tk.PhotoImage(file='icons2/light_default.png')
...
color_icon = (light_defaule_icon,light_plus_icon,dark_icon,red_icon,monokai_icon,night_blue_icon)
...
text_editer.focus_set()
scroll_bar.pack(side=tk.RIGHT,fill=tk.Y)
text_editer.pack(fill=tk.BOTH,expand=True)
scroll_bar.config(command=text_editer.yview)
...
bold_btn.configure(command=chang_bold)
...
italic_btn.configure(command=chang_italic)
...
underline_btn.configure(command=chang_underline)
...
def find_func(event=None):
    ...
    def find():
        ...
        if word:
            start_pos = '1.0'
            while True:
                start_pos = text_editer.search(word,start_pos,stopindex=tk.END)
                if not start_pos:
                    break
    ...
    # == button grid
    find_button.grid(row=2,column=0,padx=16,pady=4)
    replace_button.grid(row=2,column=1,padx=16,pady=4)
...
edit.add_command(label="Clear All",image=clear_all_icon,compound=tk.LEFT,accelerator="Ctrl+Alt+X",command=lambda:text_editer.delete(1.0,tk.END))
